...
def download_batch(data_frame,inicio, final,arr,name_dataset):
    ...
    for x in tqdm(range(inicio,final)):
        ...
        for retry in range(max_retry):
            try:
                img = get_sdss_img(data_frame, arr[x], fov=25.344/3600, size=64)
                stack.append(img)
                break
...
def download_all(df,imgs,name_dataset):
    ...
    suma_ejemplos = np.sum(imgs, axis=(1, 2, 3))
...
if __name__ == "__main__":
    ...
    t1 = threading.Thread(target=download_all, args=[df_1,imgs_1,"train9_faltantes"])
