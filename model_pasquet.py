...
class Pasquet(L.LightningModule):

    def __init__(self, config):
        super(Pasquet, self).__init__()
        ...
        self.linear_net = nn.Sequential(
                                nn.Linear(int(((config["img_size"]/8)**2)*348)+1, 1096),
                                nn.ReLU(),
                                nn.Linear(1096, 1096),
                                nn.ReLU(),
                                nn.Linear(1096, 180)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        val_zphot = (nn.Softmax(dim=1)(x_hat) * torch.linspace(0,0.4,181)[:-1].to(self.device)).sum(1)

        self.val_predictions.append(x_hat.cpu())
        self.val_classes.append(y_class.cpu())

        self.val_zphot.append(val_zphot.cpu())
    ...
    def on_validation_epoch_end(self):
        zphot = torch.cat(self.val_zphot, dim=0)
        torch.save(zphot, os.path.join(self.save_files, f"zphot_val_{self.current_epoch}.pt"))
